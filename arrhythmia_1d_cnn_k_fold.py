...
# Setting constants
DATA_PATH = '/content/mitbih_database/'
WAVELET_NAME = 'sym4'
DECIMATION_THRESHOLD = 0.04
CLASS_LABELS = ['N', 'L', 'R', 'A', 'V']
SAMPLE_COUNT_LIMIT = 10000
SAMPLE_WINDOW = 1080
KFOLDS = 5  # Number of folds for k-fold cross-validation

# Extracting the database
os.system('unzip mitbih_database.zip')
...
for train_index, test_index in kf.split(data_frame):
    train_df = data_frame.iloc[train_index]
    ...
    print(f"Fold {fold_no} - Test Loss: {performance[0]}, Test Accuracy: {performance[1]}")
...
# Printing the metrics for each fold
for metric in metrics:
    print(f"Fold {metric['fold']} - Accuracy: {metric['accuracy']}, Sensitivity: {metric['sensitivity']}, Specificity: {metric['specificity']}, Precision: {metric['precision']}, F1 Score: {metric['f1']}")

from google.colab import files

# Saving the final model
model.save('my_cnn_model.h5')
with open('my_rf_model.pkl', 'wb') as file:
    pickle.dump(rf_classifier, file)

# Download the files to your local machine
files.download('my_cnn_model.h5')
files.download('my_rf_model.pkl')
